...
function DebtRow({
  ...
  item: DebtItem;
  currency: string;
  onEdit: (i: DebtItem) => void;
  onDelete: (i: DebtItem) => void;
  isChild?: boolean;
}) {
  return (
    <div
    ...
    >
      <div className="flex items-center gap-3 w-full sm:w-auto">
        ...
        <div className="min-w-0">
          <div className={cn("font-medium truncate", isChild && "text-sm")}>
            {isChild
              ? (item.created_at?.substring(0, 10) ?? "Eintrag")
              : item.name}
          </div>
          {isChild && (
            <div className="text-xs text-muted-foreground">Einzelposten</div>
          )}
        </div>
      </div>
      ...
      <div className="flex items-center gap-4 w-full sm:w-auto justify-between sm:justify-end">
        <div
          className={cn(
            ...
            isChild ? "text-base" : "text-sm sm:text-lg",
          )}
        >
        ...
        <div className="flex items-center gap-1 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          <Button
            ...
            title="Als bezahlt markieren (Löschen)"
          >
...
export default function Debt() {
  ...
  const handleDelete = async (item: DebtItem) => {
    if (
      window.confirm(
        `Schulden von "${item.name}" wirklich als beglichen markieren (löschen)?`,
      )
    ) {
      try {
        ...
        if (item.list_id && item.payer_name) {
          ...
          if (syncError) {
            console.error(
              "Failed to sync debt status back to billings",
              syncError,
            );
